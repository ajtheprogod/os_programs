...
mem_init(void)
{
	...
	// Your code goes here:
boot_map_region(kern_pgdir, UPAGES, npages * sizeof(struct PageInfo), PADDR(pages), PTE_U | PTE_P |PTE_PS); 
...
page_init(void)
{
	...
	// free pages!
        ...
        int upper_pgnum = PGNUM(PADDR(boot_alloc(0))); 
	size_t i;
	for (i = 0; i < npages; i++) {
		 if ( (i > 0 && i < lower_pgnum) || (i >= upper_pgnum) ) { 
                        // Add to free list 
                        pages[i].pp_ref = 0; 
...
mon_showmappings(int argc, char **argv) { 
        ...
        if(lva != ROUNDUP(lva, PGSIZE) || uva != ROUNDUP(uva, PGSIZE) || lva > uva) { 
