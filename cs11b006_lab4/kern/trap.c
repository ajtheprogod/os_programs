...
trap_init(void)
{
	...
	for ( ; i <=31; i++)
            {
		SETGATE(idt[i], 0, GD_KT, trap_handlers[i], 0);//Here I handled a double fault that means if user program goes a falut 
                  //there is no handler and it goes to double fault 
	    }
