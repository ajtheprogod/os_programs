...
env_setup_vm(struct Env *e)
{
	...
	// LAB 3: Your code here.
          ...
          char *add=(char *)kern_pgdir;
          char *add1=(char *)e->env_pgdir;
          for(i=0;i<PGSIZE;i++)
              *(add1+i)=*(add+i);//COPYING ONE INTO OTHER
          
          //PP_ref should  be changed 
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	//   (Watch out for corner-cases!)
     int i;
            void *temp=ROUNDDOWN(va,PGSIZE);
            void *temp1=ROUNDUP(va+len,PGSIZE);
           for(i=0;i<(temp1-temp)/PGSIZE;i++)
            {
               struct PageInfo *page= page_alloc(!ALLOC_ZERO);
                   if(page==NULL)
                   panic("No memory\n");
               page_insert(e->env_pgdir,page,va+i*PGSIZE,PTE_U|PTE_W);
             }	
}
...
// Hints:
    ...
    for (; ph < eph; ph++) 
    {
        ...
        if(ph->p_type == ELF_PROG_LOAD)
        {
            ...
            //Now we are in the virtual address space of env so we can directly map it
            char* add =(char *)ph->p_va;
            char* add1 =(char *)binary+ph->p_offset; 
