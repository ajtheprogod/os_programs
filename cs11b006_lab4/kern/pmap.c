...
boot_alloc(uint32_t n)
{
	...
	if(n == 0) {
		...
		uint32_t to_alloc = (uint32_t)ROUNDUP((char *)n, PGSIZE);
		result = nextfree;
		nextfree += to_alloc;
		//In entrypgdir.c we can see that only 4MB has been added to the page
		//Mapped pages are VA's [KERNBASE, KERNBASE+4MB) to PA's [0, 4MB)
		if((int)nextfree > KERNBASE+4194304){
...
mem_init(void)
{
	...
	uint32_t size1=NENV*sizeof(struct Env);
	envs=boot_alloc(size1);
	memset(envs,0,size1);
	//////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
	//PTSIZE - seen from memlayout.h
	//PADDR(pages) or page2pa(pages[0])
	...
	// LAB 3: Your code here.
	boot_map_region(kern_pgdir, UENVS, ROUNDUP(size1, PGSIZE), PADDR(envs), PTE_U | PTE_P);
	//////////////////////////////////////////////////////////////////////
...
{
	// Fill this function in
	pte_t *pg_t = pgdir_walk(pgdir, va, 0);
	if(pte_store != 0){
		*pte_store = pg_t;
	}
	if(!pg_t || !(pg_t[0] & PTE_P))
		return NULL;

	uintptr_t addr = PTE_ADDR(*pg_t);
	return pa2page(addr);
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	// Reserve size bytes of vpirtual memory starting at base and
	// maa physical pages [pa,pa+size) to virtual addresses
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	int i;
	void *var;
	void * var1=(void *)ROUNDUP(((uint32_t)va)+len-1,PGSIZE);
	var=(void *)ROUNDDOWN((uint32_t)va, PGSIZE);
	perm|=PTE_P;

	for(i=(int)0;i<((uint32_t)var1-(uint32_t)var);i+=PGSIZE)
	{
		page_lookup(env->env_pgdir,(void *)((uint32_t)var+i),&a);
		if((((uint32_t)var+i)>ULIM)||((*a&perm)!=perm))
		{
			if(i==0)
				user_mem_check_addr=(uint32_t)va;
			else
				user_mem_check_addr=(uint32_t)((uint32_t)var+i);
			return -E_FAULT;
		}
	}
	return 0;
...
check_page_free_list(bool only_low_memory)
{
	...
	cprintf("check_page_free_list succedded\n");
}
